...
| API-028 | POST | `/api/v1/vms/batch-actions` | 批量电源操作（`vm_ids` + `action`）。**逐台独立受理、可部分成功**，因此始终返回 200，逐台结果在 `items` 里；失败项带具体原因。同一 ID 会被去重，单次上限 50 台 | 是 | 已实现（仅电源操作；批量删除待补） | F-2-01 |
