...
| 能力簇 | 关键能力 | 编号 |
|---|---|---|
| **节点代理（agent）** | 每台宿主机一个轻量 agent：注册与心跳、**能力自报**、领域操作执行、指标采集与本地聚合、断连自治与重连对账；是控制面与宿主机之间的**唯一通道** | F-6-08（[ADR-0005](../06-decisions/0005-control-plane-node-agent-architecture.md)） |
| 节点与集群 | 节点纳管（一次性令牌 + agent 注册）、节点状态与能力清单、节点资源清单与直通诊断、迁移目标与接管、维护模式、宿主机虚拟化调优、agent 生命周期 | F-6-01 ~ F-6-08 |
| 存储底座 | 存储池（识别/格式化挂载/分区/LVM/默认池）、存储卷、容量与配额视图（宿主侧动作由 agent 执行） | F-5-01、F-5-02、F-5-07 |
| 网络底座 | 网络后端抽象（基础/增强）、能力探测与降级、网络状态与自愈（宿主侧动作由 agent 执行） | F-4-01、F-4-13 |
| 身份与权限 | 本地登录、会话与令牌、多阶段登录、RBAC、资源归属 | F-1-01、F-1-02、F-1-03、F-1-06、F-1-09 |
| 监控采集 | 宿主机指标周期采集（含按网卡/硬盘分设备）、历史留存（agent 采样与聚合后上报） | F-8-01 |
...
| 能力 | 前置依赖 | 说明 |
|---|---|---|
| VM 创建（F-2-02） | **节点在线且 agent 可用**（F-6-01/02/08）+ 存储池（F-5-01）+ 网络后端（F-4-01）+ 配额校验（F-1-08）+ 任务队列（F-7-01） | 任一缺失应给出**可执行的修复提示**，而不是笼统失败 |
| 模板（F-3-01） | 关机态虚拟机 + 存储池 + 任务队列 | 制作中禁止修改元数据 |
| 快照（F-2-07） | 虚拟机 + 存储 + 快照配额 | 运行态自动选择内/外部快照策略 |
| 迁移（F-2-15） | **源与目标 agent 均在线**（F-6-01/08）+ 目标接管（F-6-04）+ 存储/网络匹配 | 预检不通过即阻断并给出原因与阈值；数据搬运由两侧 agent 完成 |
...
| 跨切面 | 承担的能力 | 约束 |
|---|---|---|
| **节点通道** | F-6-08（[ADR-0005](../06-decisions/0005-control-plane-node-agent-architecture.md)） | 控制面与宿主机之间的**唯一路径**：领域操作 + 任务指令 + 进度/事件上行；连接注册、能力缓存、心跳与"陈旧数据"语义都在此统一实现，其他能力不得自建旁路 |
...
宿主侧依赖由**节点 agent 自探测并上报**（[ADR-0005](../06-decisions/0005-control-plane-node-agent-architecture.md) §6），控制面据此启用或降级。

| 能力 | 宿主侧依赖 | 来宾侧依赖 | 缺失时 |
|---|---|---|---|
| 节点接入（F-6-01/08） | agent 已安装，且可出向连通控制面 | — | 未注册的节点不可用；界面给出安装与排障指引 |
| 虚拟机创建与电源（F-2-02/04） | libvirt、qemu、磁盘工具、网桥或 OVS（由 agent 自报） | — | agent 上报能力缺失后由控制面拦截，不落期望态 |
| 快照（F-2-07） | 磁盘格式与快照支持 | — | 降级为外部快照或禁用创建并说明 |
| 迁移（F-2-15） | **源与目标 agent 均在线**、复制通道（agent↔agent 或控制面中转，见 [`ARCHITECTURE.md`](../02-architecture/ARCHITECTURE.md) §8） | — | 阻断并给出预检原因与阈值 |
...
| §4 宿主机与节点 | 节点纳管、**能力自报**、资源清单、调优、agent 生命周期（参考项目的「API + SSH 双通道」由 agent 架构**替代**，见 [ADR-0005](../06-decisions/0005-control-plane-node-agent-architecture.md)） | F-6-01 ~ F-6-03、F-6-07、F-6-08 |
| §4 系统设置 | 系统设置 | F-9-01 |
...
| 阶段 | 主题 | 覆盖能力 | 完成标志 |
|---|---|---|---|
| M2 最小可用 | 底座打通 + 单机完整生命周期 | **agent 纳管与节点通道** + L0 其余 + VM 列表/创建/详情/电源/控制台 + 任务与实时通道 + 设置与安全基础（P0） | 能纳管一台宿主机，并完成"建 VM → 开机 → 控制台操作 → 关机 → 删除"闭环 |
...
3. **跨切面不自建**：任务、**节点通道**、实时通道、高风险验证、配额、审计必须复用 §4 的既有实现；**任何"绕过 agent 直接操作宿主机"的做法都属越界**（见 [ADR-0005](../06-decisions/0005-control-plane-node-agent-architecture.md)）。
...
| 2026-09-15 | 按 [ADR-0005](../06-decisions/0005-control-plane-node-agent-architecture.md) 同步：L0 新增「节点代理（agent）」能力簇并补 `F-6-08`；依赖链与运行态前提改为「agent 在线 / 能力自报」口径（去掉目标节点 SSH root）；跨切面新增「**节点通道**」；覆盖度对照与阶段视角同步 |
